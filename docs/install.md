...
1. 取得用户选择的源码版本并记录 ref/commit；未指定时可使用默认分支，已有源码可复用：
   ...
   入口按脚本自身位置定位源码；也可从任意目录执行 `sh "/路径/含 空格或中文的源码/setup.sh"`。
   帮助、安装、回滚和卸载均不要求预装本项目或设置 `PYTHONPATH`。
...
Setup 仅在 Linux 运行，自动 APT 准备范围为 **Ubuntu 24.04、Debian 13 的 x86_64 / aarch64**，且 `apt-get` 可用。
这两代基础发行版提供 CPython 3.11+ 和 Git 2.40+；仍逐项检测实际工具，已有兼容版本直接复用。
Ubuntu 22.04、Debian 12、衍生发行版及其他架构不自动补装；即使已有自定义软件源，也明确列出缺项供人工准备，继续可行的安装步骤。

GitHub CLI 不在自动 APT 补装范围：Ubuntu 24.04 原生源中的 gh 2.45.0 缺少所需的 `gh api --slurp`。已有兼容 gh 继续复用；缺失或不兼容时，按下表官方说明人工安装或升级，确认 `gh api --help` 包含 `--paginate`、`--slurp`、`--method`、`--header` 后重跑 Setup。Setup 不添加 GitHub CLI 软件源，也不把重复安装旧候选当作修复。

一次展示缺项、APT 候选来源和安装计划，确认后仅补齐列出的依赖，不添加软件源、不升级整个系统、不替用户安装或登录 Codex。
仅使用当前 root 权限或已有的非交互 `sudo -n` 授权；`--yes` 不触发密码登录或取得额外授权。
缺权限时显示管理员操作；缺认证时提示自行登录；缺 user systemd 或 namespace 能力时提示宿主修复，不改变全机安全策略。
软件源候选不可用、版本仍不兼容或包安装失败时按实际错误处理，不能视为执行就绪。

上述范围是自动准备策略，模拟工具测试只验证分支与授权边界，不是完整平台支持证明。
本轮宿主证据：Ubuntu 24.04 x86_64，CPython 3.11.16、Git 2.43.0、bubblewrap 0.9.0。
真实 `bwrap --die-with-parent --ro-bind / / --dev /dev --unshare-pid --proc /proc -- /bin/true` 成功；
`systemctl --user show-environment` 返回 `Failed to connect to bus: Connection refused`，因此 user systemd 未就绪。
未调用真实模型；完整执行兼容性、Debian 13 与 aarch64 尚未验证，不宣称完整宿主通过。
...
构建或激活失败保留原版本。先按错误中的阶段、原因和下一步修复，再从同一源码重跑；
不要删除旧安装或手工建立虚拟环境。安装成功但 doctor 未就绪时，已激活版本仍保留，补齐宿主条件后重跑 Setup。
安装后的资源随包交付，日常命令不依赖源码目录；更新和管理仍从所选源码入口执行。
