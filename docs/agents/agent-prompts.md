...
当前正文的权威来源为 `src/agent_run/resources/{zh,en}/`。五份 `methods/*.md` 分别定义开发共用、初次开发、修复、验收和发布方法。个人目录为 `$XDG_CONFIG_HOME/agent-run/prompts/<language>/`（未设置时使用 `~/.config/agent-run/prompts/<language>/`），文件名与内置方法一致，跨项目共用。程序读取完整文件，不按标题或章节路由。

初次开发组合共用与初次开发；新修复工作无论复用线程还是新建线程，都组合共用与修复，不保留隐藏的紧凑方法。同一语义轮的执行续接使用内部短指令，沿用已建立的方法。角色权限、任务事实、输出格式与探针由程序内部 Markdown 提供，不能由方法替换授予 Git/GitHub 写权限。Skill 选择属于可定制方法。

初始化只补缺失的五份方法，保留已有文件；缺少个人文件只回退同语言当前内置版本，不跨语言借用或自动翻译，存在但不可读明确失败。差异查看只读，不自动合并；个人副本不会自动继承升级后的默认正文。所有角色与预览使用统一资源读取入口。

个人 `settings configure --language zh|en` 是唯一语言配置；默认中文且不依赖宿主 locale。每个新 Run 固定语言和全部所选静态资源内容，后续配置或内置正文变化仅影响新 Run；当前对象、工作区、证据和人工回复仍逐次注入。没有固定语言或所需资源快照的旧状态明确不兼容，不猜测填充。不保存逐次完整动态 Prompt 或对话。Agent 自然语言输出仅由方法引导，合法另一语言或混合语言输出原样接受，不增加语言检测、拒收、纠正、翻译或额外模型调用。原始标题、用户反馈及技术证据不翻译。短界面文案集中于同一语言目录的 `messages.json`，与长 Markdown 按职责分开。以下中文样稿用于解释两种语言共有的角色合同；实际完整文本以资源及真实组装结果为准。
...
- 内置方法沿用适用 Skill，个人方法可以选择或替换 Skill 使用方式。内部 Prompt 写清角色、任务、权威输入、角色特有责任、完成条件与唯一交付物；
...
- 新角色、新对象或新会话接收完整角色合同并重新读取需求。同会话的新修复任务也接收共用与修复方法；
