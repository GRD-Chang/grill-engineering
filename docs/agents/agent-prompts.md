...
状态：2026-09-19 中文资源合同已同步。

## 中文资源与个人方法

当前正文的权威来源为 `src/agent_run/resources/zh/`。五份 `methods/*.md` 分别定义开发共用、初次开发、修复、验收和发布方法。个人目录为 `$XDG_CONFIG_HOME/agent-run/prompts/zh/`（未设置时使用 `~/.config/agent-run/prompts/zh/`），文件名与内置方法一致，跨项目共用。程序读取完整文件，不按标题或章节路由。
...
初始化只补缺失的五份方法，保留已有文件；缺少个人文件回退当前内置版本，存在但不可读明确失败。差异查看只读，不自动合并；个人副本不会自动继承升级后的默认正文。所有角色与预览使用统一资源读取入口。

每个新 Run 固定全部静态资源内容，后续配置或内置正文变化仅影响新 Run；当前对象、工作区、证据和人工回复仍逐次注入。没有所需资源快照的旧状态明确不兼容，不猜测填充。不保存逐次完整动态 Prompt 或对话。以下样稿用于解释角色合同；实际完整文本以资源及真实组装结果为准。
