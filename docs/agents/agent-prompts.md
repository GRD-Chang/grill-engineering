...
状态：2026-09-20 中英文资源及 Run 固定语言合同已同步。

## 双语资源与个人方法

用户方法正文的权威来源为 `src/agent_run/resources/{zh,en}/`；内部固定文案由代码按职责维护，中英文同用途配对。四份 `methods/*.md`（`development.md`、`repair.md`、`acceptance.md`、`publishing.md`）分别完整定义开发、修复、验收和发布的职责、方法、验证要求与完成标准。个人目录为 `$XDG_CONFIG_HOME/agent-run/prompts/<language>/`（未设置时使用 `~/.config/agent-run/prompts/<language>/`），文件名与内置方法一致，跨项目共用。程序读取完整文件，不按标题或章节路由。

初次开发使用完整开发正文；新修复工作无论复用线程还是新建线程，都使用完整修复正文，不保留隐藏的紧凑方法。同一语义轮的执行续接使用内部短指令，沿用已建立的方法。角色权限、任务事实、输出格式与探针由程序内部文案与输出 schema 提供，不能由方法替换授予 Git/GitHub 写权限。Skill 选择属于可定制方法。

初始化只补缺失的四份主体，保留已有文件；缺少个人文件只回退同语言当前内置版本，不跨语言借用或自动翻译，存在但不可读、空白或无效编码明确失败。差异查看只读，不自动合并；个人副本不会自动继承升级后的默认正文。所有角色与预览使用统一资源读取入口。

个人 `settings configure --language zh|en` 是唯一语言配置；默认中文且不依赖宿主 locale。每个新 Run 固定语言和四份主体与全部所选语言的内部静态文案，后续配置或内置正文变化仅影响新 Run；当前对象、工作区、证据和人工回复仍逐次注入。没有固定语言或所需资源快照的旧状态明确不兼容，不猜测填充。不保存逐次完整动态 Prompt 或对话。Agent 自然语言输出仅由方法引导，合法另一语言或混合语言输出原样接受，不增加语言检测、拒收、纠正、翻译或额外模型调用。原始标题、用户反馈及技术证据不翻译。短界面文案集中于同一语言目录的 `messages.json`，与长 Markdown 按职责分开。以下中文样稿用于解释两种语言共有的角色合同；其中 JSON 样例仅用于说明接口，不是运行时复制的第二份结构定义。实际完整文本以代码文案、用户主体及真实组装结果为准。输出结构由 `agent_schemas.py` 定义并独立传入模型，`artifacts.py` 校验业务语义；内部输出指导保留证据要求和结果语义，不手写完整 JSON 样例。
...
文案维护位置和可读性核对范围见[资源清单](prompt-resource-map.md)。旧格式个人文件保留、提示对应的新主体及实际生效来源，由用户自行迁入；不自动合并，也不作为新主体使用。
...
角色正文直接调用运行时生成函数；安装器兼容性检查和文案 schema 握手的展示也读取同一内置文案入口。
实际安装探针读取候选安装包自己的文案；展示脚本使用当前源码。必需资源缺失时明确报错，避免复制正文后漂移。它不启动 Codex、不访问 GitHub、不读取真实运行状态，
也不包含 Codex 自身系统指令、自动加载的 `AGENTS.md` 或 Skill 内容。默认只输出到终端，
只有指定 `--output` 时才写入展示文件。

## 编写与组装原则

- 内置方法沿用适用 Skill，个人方法可以选择或替换 Skill 使用方式。主要正文写清角色、方法、角色特有责任与完成条件；程序按当前事实附加任务、权威输入、固定权限与唯一交付物合同；
  不重复 Skill 已有且适用的方法，冲突时说明具体替代哪一步。
- 角色特有责任不限于冲突项。例如程序统一提交工作树，因此开发必须知道清理责任及其原因。
- Worker 正文使用所选语言的直接工作用语（默认中文，支持英文）；字段、命令、Skill 名称保留原文。
...
- 新角色、新对象或新会话接收完整角色合同并重新读取需求。同会话的新修复任务也接收完整修复正文；
  同一任务的执行续接使用角色化短样稿。仅修复输出格式时不重新开展语义工作。
- 按开发轮使用新会话时，开发与定向修复均标明当前 checkout，继续已有及未提交代码，不清空工作区。
  新会话自行读取权威需求建立本轮基线；Controller 摘要、旧开发总结、历史对话与旧验收结论不能
  替代权威需求和当前证据。定向修复保留原始修复证据与适用人工回应，仍按修复权限和验证责任完成，
  不重新取得从零开发或开发侧预检职责。会话策略、预算与模型绑定由程序处理，不注入员工任务说明。
...
已有会话开始新的修复任务时同样注入共用与修复方法；以下展示职责要点，不能作为另一份内部精简方法覆盖个人选择：
...
同会话的新修复工作与中断后继续完成同一任务的短指令是不同分支。
