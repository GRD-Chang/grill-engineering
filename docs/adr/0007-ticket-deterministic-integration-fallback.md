...
Ticket、Run 与 Parent-only 的每次 Reviewer 都按 Prompt 调用 `code-review` skill。每个窗口的 Reviewer 1 建立当前基线；Ticket Reviewer 2–3、Run Reviewer 2–N+1（默认 N=10 时为 R2–R11）以及 Parent-only Reviewer 2–N 只额外收到紧邻上一轮 Acceptance Artifact 的完整原始内容及其角色化 review identity，优先核销原 Findings、检查上一验收对象到当前对象的 repair delta 与直接回归；缺少具体风险依据时避免对未变化代码重复完整扫描，当前证据或实际影响需要时可自主扩大范围。Ticket/Parent-only 使用 reviewed base/Candidate，普通 Run 使用 default base、Run head 与 expected merge tree，Run Repair Candidate 使用 Run base、Repair Candidate 与 expected merge tree。Reviewer 看到当前与剩余自动验收次数，定向 Repair 看到已完成与剩余次数；这只是现有预算的角色化投影，不改变门禁。所有审查或评价型 subagent 使用 `fork_turns: "none"`。Controller 不解析 Findings、不维护 closure 状态，也不要求 Development 逐项报告处理结果。Run 的 Review-first D10/R11 策略、CI Failure Evidence、Repair Cycle、人工批准或最终完成权威不受本 ADR 的 Ticket fallback 机制影响；Parent-only 的旧 Review 决策由 Issue #170 的后续取代决策替换，具体见下节。
...
该取代只适用于 Parent-only 的预算上限、配对拓扑与检查点路径，不改变本 ADR 已接受的 Ticket `D4/R3` Deterministic Fallback、Final CI-fix、Required Checks、Run Acceptance 或 Publisher 权威边界；Run 的 Review-first 预算、CI Failure Evidence、Repair Cycle、人工批准或最终完成权威也不由 Issue #170 改变。
...
删除本地代码 Validation 后，未配置 Required Checks 的兜底路径没有代码级测试证明；系统接受这一 thin-harness 取舍，但必须在状态、Deterministic Integration Record 与最终 Run Acceptance 输入中明确暴露 Required Checks Observation 的 `result=none`，不能伪装成 CI 通过。普通 Fresh Acceptance 路径仍沿用仓库实际 CI 策略。Ticket Completion 只表示已进入 Run Branch，不表示最终交付。Run Acceptance 必须读取并验证兜底证据，且仍是进入默认分支前不可跳过的完整语义验收；Run 自身每个 Review Budget Window 最多 `N+1` 次 Reviewer Invocation（默认 R11），第 `N+1` 次失败后只允许人工 `resume` 开启新窗口，不使用 Ticket 兜底。
